...
std::vector<std::pair<std::uint64_t, std::uint64_t>> can_moves{
    // Peg 33
...
bool solve(std::uint64_t board, std::size_t pegs) {
  ++counter;

  if (pegs == 1) {
    std::cout << "counter: " << counter << "pegs: " << pegs << std::endl;
    print_board(board);
    return true;
  }

  /*else {*/
  /*  std::cout << "number of pegs: " << __builtin_popcountll(board) << std::endl;*/
  /*}*/

  for (const auto& can_move : can_moves) {
    if (((board ^ can_move.first) & can_move.second) == 0 && solve(board ^ can_move.second, pegs - 1)) {
      print_board(board);
      return true;
    }
  }
