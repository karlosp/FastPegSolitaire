...
constexpr const std::array<std::pair<std::uint64_t, std::uint64_t>, 76>
    can_moves{
        // Peg 33
        std::pair<std::uint64_t, std::uint64_t>{p33 | p30,
                                                p33 | p30 | p25},  // 33 down
...
constexpr const std::array<std::pair<std::uint32_t, std::uint32_t>, 72>
    can_moves_32{
        // Peg 33
        /*std::pair<std::uint32_t, std::uint32_t>{pr33 | pr30, pr33 | pr30 |
           pr25},  // 33 down*/
        /*{pr33 | pr32, pr33 | pr32 | pr31}, // 33 right*/
        // Peg 32
        std::pair<std::uint32_t, std::uint32_t>{pr32 | pr29,
                                                pr32 | pr29 | pr24},  // 32 down
        // Peg 31
...
std::array<std::uint64_t, 32> solved_boards{
    0ul, 0ul, 0ul, 0ul, 0ul, 0ul, 0ul, 0ul, 0ul, 0ul, 0ul,
    0ul, 0ul, 0ul, 0ul, 0ul, 0ul, 0ul, 0ul, 0ul, 0ul, 0ul,
    0ul, 0ul, 0ul, 0ul, 0ul, 0ul, 0ul, 0ul, 0ul, 0ul};
int count_empty = 0;

constexpr bool solve(const std::uint64_t board, const std::size_t pegs) {
//  ++counter;
...
int main() {
    constexpr auto board = //0b000011000000000000000000000000001; 
        0b111111111111111101111111111111111;  // One empty spot in the center

    constexpr auto pos = 7;
    constexpr auto can_move = (board & can_moves[7].second) == can_moves[7].first;

    int count = 0;
    //for (const auto& cm : can_moves) {
      //  ++count;
    //}
    constexpr auto res = solve(board, 32);
    print_board(solved_boards[0]);
    return res;
}
